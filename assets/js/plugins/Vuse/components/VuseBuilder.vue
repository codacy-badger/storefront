<template>
    <div>
        <div class="artboard" id="artboard" ref="artboard"
             :class="{ 'is-sorting': $builder.isSorting, 'is-editable': $builder.isEditing }">
            <component v-for="section in $builder.sections" :is="section.name" :key="section.id"
                       :id="section.id"></component>
        </div>
        <div class="controller">
            <div class="controller-intro" v-if="showIntro &amp;&amp; !this.$builder.sections.length">
                ...
                <template v-if="themes">
                    <div class="controller-themes">
                        <button class="controller-theme" v-for="theme in themes" @click="addTheme(theme)">{{ theme.name
                            }}
                        </button>
                    </div>
                </template>
            ...
            <div class="controller-panel">
                ...
                <button class="controller-button is-red" v-if="!tempSections" tooltip-position="top"
                        tooltip="clear sections" @click="clearSections">
                    <VuseIcon name="trash"></VuseIcon>
                </button>
                <button class="controller-button is-gray" v-if="tempSections" tooltip-position="top" tooltip="undo"
                        @click="undo">
                    <VuseIcon name="undo"></VuseIcon>
                </button>
                <button class="controller-button is-blue" tooltip-position="top" tooltip="sorting"
                        :class="{ 'is-red': $builder.isSorting }" @click="toggleSort">
                    <VuseIcon name="sort"></VuseIcon>
                </button>
                <button class="controller-button is-blue" tooltip-position="top" tooltip="add section"
                        :class="{ 'is-red': listShown, 'is-rotated': listShown }" :disabled="!$builder.isEditing"
                        @click="newSection">
        ...
        <ul class="menu" :class="{ 'is-visiable': listShown }" ref="menu">
            <li class="menu-group" v-for="(group, name) in groups" v-if="group.length">
                <div class="menu-header" @click="toggleGroupVisibility"><span class="menu-title">{{ name }}</span><span
                        class="menu-icon">
          <VuseIcon name="arrowDown"></VuseIcon></span></div>
                <div class="menu-body">
                    <template v-for="section in group"><a class="menu-element" @click="addSection(section)"
                                                          @drag="currentSection = section"><img
                            class="menu-elementImage" v-if="section.cover" :src="section.cover"/><span
                            class="menu-elementTitle">{{ section.name }}</span></a></template>
                </div>
            </li>
        </ul>
    </div>
